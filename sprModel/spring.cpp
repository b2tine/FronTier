	...
	Front *front)
{
	...
	for (i = 0; i < num_curves; ++i)
	{
	    ...
	    hsbdry_type(curves[i]) = FIXED_CURVE;
	    ...
	    FT_InitCurveVeloFunc(curves[i], "preset_vfunc", 
				(POINTER)&vparams[i], node_vel_func);
	}
	...
	Front *front)
{
	...
	for (i = 0; i < num_nodes; ++i)
	{
	    ...
	    vparams[i].time = &front->time;
	    FT_InitNodeVeloFunc(nodes[i], "preset_vfunc", (POINTER)&vparams[i], 
					node_vel_func);
	}
	...
	double dt)
{
	...
	if (hsbdry_type(oldc) != FIXED_CURVE || oldc->vel_pack == NULL) 
	{
	    if (debugging("trace"))
	...
	dim = FT_Dimension();
        vel_func_pack = (VELO_FUNC_PACK*)oldc->vel_pack;
        vparams = vel_func_pack->func_params;
        vfunc = vel_func_pack->func;
        (*vfunc)(vparams,front,NULL,NULL,NULL,vel);
	...
	double dt)
{
	...
	if (debugging("trace"))
	    (void) printf("Entering spring_node_propagate()\n");
	if (oldn->vel_pack == NULL) 
	{
	    if (debugging("trace"))
	    	(void) printf("Leaving spring_node_propagate()\n");
	    return;
	}

	dim = FT_Dimension();
        vel_func_pack = (VELO_FUNC_PACK*)oldn->vel_pack;
        vparams = vel_func_pack->func_params;
        vfunc = vel_func_pack->func;
        (*vfunc)(vparams,front,NULL,NULL,NULL,vel);
