	...
	double *var_out)
{
	...
	double C_nb,lambda,eta,coeff,coeff_nb,rhs,v;
	COMPONENT comp;
	PETSc solver;
	double *x;
        int num_iter = 0;
	...
	for (j = jmin; j <= jmax; ++j)
	{
	    ...
	    for (l = 0; l < dim; ++l)
	    {
		    ...
		    {
			coeff_nb = -lambda;
			v = (*getStateVel[l])(state);
			eta = 0.5*v*sub_dt/top_h[l];
...
void PARABOLIC_SOLVER::solveCEX()
{
	...
	double kappa[3][2];	// side regularity number:
				// 1: for regular; 0: for irregular
	double sink;
	double vel0[MAXD],vel_nb,adv_flux[MAXD][2],net_flux;
	boolean irregular;
	double coords[MAXD];
	...
	switch (dim)
	{
		...
		for (l = 0; l < dim; ++l)
		{
			...
			fr_crx_grid_seg = (*findStateAtCrossing)(front,icoords,
			    ...
			    solute_nb[m] = var[icn];
			    if (a != NULL)
			    	adv_flux[l][m] = 0.5*(vel0[l]*solute + 
					a[l][icn]*solute_nb[m]);
                        }
			else if (fr_crx_grid_seg == DIRICHLET_PDE_BOUNDARY)
			{
			    ...
			    else
			    {
			    	...
			    	if (a != NULL)
				{
			    	    vel_nb = (*getStateVel[l])(state);
			    	    adv_flux[l][m] = vel_nb*solute_nb[m];
				}
			    }
			}
			else if (fr_crx_grid_seg == MOVING_BOUNDARY ||
			    fr_crx_grid_seg == NEUMANN_PDE_BOUNDARY)
			{
			    irregular = YES;
			    kappa[l][m] = 0.0;
			}
			...
			{
			    (void) printf("Unknown PDE boundary type\n");
                            clean_up(ERROR);
			}
                        dgrad[l] += kappa[l][m]*(solute_nb[m] - solute)
						/top_h[l];
                    }
		    net_flux += (adv_flux[l][1] - adv_flux[l][0])/top_h[l];
		}
	...
	switch (dim)
	{
		...
		else if (top_comp[ic] == obst_comp)
		{
		    ...
		    else
		    {
		    	...
		    	soln[ic] = (*var_obst_func)(obst_func_params,coords);
		    }
		}
	    }
	    break;
...
void PARABOLIC_SOLVER::CCNsolve1d(void)
{
	int i,j,k,l,m,ic,icn,I,I_nb,icoords[MAXD];
	int gmin[MAXD],ipn[MAXD];
	double crx_coords[MAXD];
	...
	int size;
        double rel_residual = 0;
        int fr_crx_grid_seg;
        const GRID_DIRECTION dir[1][2] =
                {{WEST,EAST}};
	HYPER_SURF *hs;
	POINTER state;
        double v_new,v_old,v0;                  // cell volumes.
        double eta_new,eta_old; // cell irregularity ratio
        double kappa[1][2];     // side regularity number:
                                // 1: for regular; 0: for irregular
	double sink;
...
void PARABOLIC_SOLVER::CCNsolve2d(void)
{
	int i,j,k,l,m,ic,icn,I,I_nb,icoords[MAXD];
	int gmin[MAXD],ipn[MAXD];
	double crx_coords[MAXD];
        double crx_coords_old[MAXD];
	double C0,C_nb,eta,eta_nb,lambda,coeff,coeff_nb,rhs;
	COMPONENT comp;
	PETSc solver;
	double *x;
        int num_iter = 0;
	...
	POINTER state;
        double v_new,v_old,v0;                  // cell volumes.
        double eta_new,eta_old; // cell irregularity ratio
        double kappa[2][2];     // side regularity number:
                                // 1: for regular; 0: for irregular
	double sink;
	boolean regular_nb;
        double vel;
	    ...
	    for (l = 0; l < dim; ++l)
	    {
			...
			if (nu == NULL)
			    ...
			    lambda = 0.5*(nu[ic] + nu[icn])*
                	...
                	if (a != NULL)
                        {
                            eta = 0.5*a[l][ic]*dt/top_h[l];
                            eta_nb = 0.5*a[l][icn]*dt/top_h[l];
                            if (m == 0)
                            {
                                eta *= -1.0;
                                eta_nb *= -1.0;
		    	...
		    	coeff_nb = -0.5*lambda + 0.5*eta_nb;
			solver.Add_A(I,I_nb,coeff_nb);
			coeff += 0.5*lambda + 0.5*eta;
			C_nb = var[icn];
			rhs -= -0.5*lambda*(C_nb - C0);
			rhs += -0.5*(eta_nb*C_nb + eta*C0);
                    }
		    else
		    {
		    	fr_crx_grid_seg = (*findStateAtCrossing)(front,icoords,
			...
			if (fr_crx_grid_seg == NO_PDE_BOUNDARY)
                        {
                            (void) printf("ERROR: no crossing between current "
                                          "and irregular neighbor cells\n");
                            clean_up(ERROR);
                        }
			else if (fr_crx_grid_seg == DIRICHLET_PDE_BOUNDARY)
			{
				...
				vel = (*getStateVel[l])(state);
                                ...
                                eta = (m == 0) ? -vel*dt/top_h[l] :
                                        vel*dt/top_h[l];

			    	coeff += 0.5*lambda;
				coeff_nb = -lambda + eta;
				rhs -= coeff_nb*C_nb + 0.5*lambda*C0;
			    }
			}
			else if (fr_crx_grid_seg != NEUMANN_PDE_BOUNDARY &&
                                 fr_crx_grid_seg != MOVING_BOUNDARY)
...
void PARABOLIC_SOLVER::CCNsolve3d(void)
{
	int i,j,k,l,m,ic,icn,I,I_nb,icoords[MAXD];
	int gmin[MAXD],ipn[MAXD];
	double crx_coords[MAXD];
...
void PARABOLIC_SOLVER::Csolve2d(
	...
	double *var_out)
{
	int i,j,l,m,ic,icn,I,I_nb,icoords[MAXD];
	int gmin[MAXD],ipn[MAXD];
