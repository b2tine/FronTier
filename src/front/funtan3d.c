	...
	int side)
{
	...
	{
	    if (wave_type(*s) != w_type || is_bdry(*s)) 
		continue;
	    to_delete = NO;
	    surf_tri_loop(*s,tri)
	    {
	...
	if (debugging("delete_surf_on_side"))
	{
	    consistent_interface(intfc);
	    (void) printf("Passed second consistent_interface()\n");
...
EXPORT	int color_bounded_surf_pieces(
	SURFACE *surf)
{
	int i,num_pieces;
	TRI *tri,**open_tri;
	...
	/* reset all tris on surface */
	surf_tri_loop(surf,tri)
	    Tri_color(tri) = -1;
	uni_array(&open_tri,surf->num_tri,sizeof(TRI*));
	
	num_pieces = 0;
	tri = open_tri[++n] = first_tri(surf);
	Tri_color(tri) = num_pieces;
	seed_found = YES;
...
start_loop:
		for (i = 0; i < 3; ++i)
		{
		    if (is_side_bdry(open_tri[n],i) ||
			Tri_on_side(open_tri[n],i) == NULL) 
			continue;
		    tri = Tri_on_side(open_tri[n],i);
		    if (Tri_color(tri) != -1) continue;
		    Tri_color(tri) = num_pieces;
		    open_tri[++n] = tri;
		    goto start_loop;
		}
	...
	surf_tri_loop(surf,tri)
	    if (Tri_color(tri) == -1)
		printf("Error: tri not painted!\n");
	free_these(1,open_tri);
	return num_pieces;
}		/*end color_bounded_surf_pieces*/

EXPORT boolean split_surf_with_colors(SURFACE* olds, int ncolors) {
        int             color, n;
        SURFACE         **news;
        CURVE           **neg, **pos, **c;
        TRI             *tri, *ntri;
        ...
        BOND            *b;
        boolean         sav_intrp;
	INTERFACE	*intfc = olds->interface;

        news = NULL;
        neg = pos = NULL;
        uni_array(&news, ncolors, sizeof(SURFACE*));

        for (color = 0; color < ncolors; ++color)
        {
            c = NULL;
            neg = pos = NULL;
            /*find postive curves and negtive curves in old surface*/
            surf_pos_curve_loop(olds, c) {
                b = (*c)->first;
                for (bts = Btris(b); bts && *bts; ++bts)
                {
                    tri = (*bts)->tri;
                    if (Tri_index(tri) == color)
                    {
                        unique_add_to_pointers(*c, &pos);
                        break;
                    }
                }
            }

            surf_neg_curve_loop(olds, c) {
                b = (*c)->first;
                for (bts = Btris(b); bts && *bts; ++bts)
                {
                    tri = (*bts)->tri;
            ...
            /*create new surfaces*/
            news[color] = copy_surface(olds, neg, pos, NO);
        }

	for (color = 0; color < ncolors; ++color)
        {
            /*move tris from old surface to new surface*/
            for (tri = first_tri(olds); !at_end_of_tri_list(tri, olds);
                 tri = ntri)
            {
                ntri = tri->next;
                if (Tri_index(tri) == color)
                {
                    remove_tri_from_surface(tri,olds,YES);
                    insert_tri_at_tail_of_list(tri,news[color]);
                }
            }
        }
        ...
        if (olds->num_tri == 0)
        {
            while(olds->pos_curves != NULL)
                (void) remove_curve_from_surface_bdry(olds,
                        olds->pos_curves[0],POSITIVE_ORIENTATION);
            while(olds->neg_curves != NULL)
                (void) remove_curve_from_surface_bdry(olds,
                        olds->neg_curves[0],NEGATIVE_ORIENTATION);
            if (!delete_surface(olds))
            {
                (void) printf("WARNING in "
                              "split_surf_with_colors(), "
                              "can't delete old surface %llu\n",
                              (long long unsigned int)
                              surface_number(olds));
                if (debugging("split_surface"))
                    printf("Leaving split_surf_with_colors()\n");
                return NO;
            }
        }
        else
        {
            (void) printf("WARNING in split_surf_with_colors(), "
                          "triangles remain on surface to be deleted\n");
            if (debugging("split_surface"))
                (void) printf("Leaving split_surf_with_colors()\n");
            return NO;
        }
        intfc_curve_loop(intfc, c)
            correct_curve_surface_orientation(*c);
        ...
        /*free memory*/
        FT_FreeThese(1, news);
	gview_plot_interface("intfc", intfc);
        return YES;
}
