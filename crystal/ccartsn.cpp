...
void C_CARTESIAN::computeAdvection(void)
{
	...
	parab_solver.getStateVel[0] = getStateXvel;
	parab_solver.getStateVel[1] = getStateYvel;
	parab_solver.getStateVel[2] = getStateZvel;
	parab_solver.findStateAtCrossing = findStateAtCrossing;
	...
	if (cRparams->crystal_dens_func)
	{
	    parab_solver.var_obst_func = cRparams->crystal_dens_func;
	    parab_solver.obst_func_params = cRparams->crystal_dens_params;
	}
	else
	{
	    parab_solver.var_obst = cRparams->rho_s;
	    parab_solver.var_obst_func = NULL;
	}
	...
	if (debugging("neg_solute"))
	{
		...
		{
		    ic = d_index2d(i,j,top_gmax);
		    if (field->solute[ic] < -0.001)
		    {
			printf("At i = %d j = %d: solute = %20.14f\n",i,j,
				field->solute[ic]);
			printf("m_dt = %f\n",m_dt);
			clean_up(ERROR);
		    }	    
...
void C_CARTESIAN::timeStepAnalysis(boolean outputData)
{
	...
	if (first) max_step = front->step;	/* Could be restart */
	if (max_step <= n)
	{
	    max_step = n + 1000;
	    FT_MatrixMemoryAlloc((POINTER*)&tmp_data,10,max_step,
                        sizeof(double));
	    if (max_step != 1000 && !first) 	/* not first time */
	    {
		for (i = 0; i < 10; ++i)
		for (j = 0; j < n; ++j)
		{
	...
	if (dim == 3) return;	// Not yet implemented
	...
	{
	    printf("\nStep %d: relative solute diff = %20.14g\n",n,
				time_data[7][n]/total_matter);
	    ...
	    printf("total_solute = %20.14f\n",total_solute);
	    printf("total_crystl = %20.14f\n",total_crystl);
	    printf("total_matter = %20.14f\n",total_matter);
	    printf("total_influx = %20.14f\n",total_influx+total_matter0);
	}
	...
	*vol_solute = *vol_crystl = 0.0;
	...
	case 2:
		...
		if (top_comp[ic] == SOLUTE_COMP)
		{
		    ...
		    *total_solute += field->solute[ic]*cell_vol;
		    *vol_solute += cell_vol;
		    if (field->solute[ic] < min_solute)
		    {
			min_solute = field->solute[ic];
			icrds_min[0] = i;
			icrds_min[1] = j;
		    }
		    if (field->solute[ic] > max_solute)
		    {
			max_solute = field->solute[ic];
			icrds_max[0] = i;
			icrds_max[1] = j;
		    }
		}
	...
	printf("max_solute = %f  occuring at (%d, %d)\n",
		max_solute,icrds_max[0],icrds_max[1]);
	printf("min_solute = %f  occuring at (%d, %d)\n",
		min_solute,icrds_min[0],icrds_min[1]);
...
void C_CARTESIAN::computeSource()
{
	...
	for (i = 0; i < size; ++i)
	{
	    if (debugging("conservation"))
	    {
	    	if (!InDomainIndex(i)) continue;
	    	if (top_comp[i] == SOLUTE_COMP || 
		    top_comp_old[i] == SOLUTE_COMP)
		{
		    getRectangleCenter(i,coords);
		    if (cRparams->crystal_dens_func != NULL)
                    	rho_s = (*cRparams->crystal_dens_func)(
                    	...
                    	rho_s = cRparams->rho_s;
		    source0[i] = rho_s*(cell_part[i].vol_new[1] -
	                                cell_part[i].vol_old[1]);
		    if (InDomainIndex(i))
		    	source_org += source0[i];
		}
	    }
	}
	...
	for (ic = 0; ic < size; ++ic)
		...
		{
		    printf("Warning: cannot place solute!\n");
		}
	    }
	    if (top_comp[ic] == SOLUTE_COMP && 
	...
	FT_ParallelExchGridArrayBuffer(field->solute,front,NULL);
	pp_global_sum(&source_sum,1);
	if (debugging("conservation"))
	{
	    (void) printf("source_org = %20.14g\n",source_org);
	    (void) printf("source_sum = %20.14g\n",source_sum);
	}
}	/* end computeSource */
...
boolean C_CARTESIAN::InDomainIndex(
	int ic)
{
	int icoords[MAXD];
	invert_to_icoords(ic,icoords,top_gmax,dim);
	if (icoords[0] < imin || icoords[0] > imax) return NO;
	if (dim == 2 && (icoords[1] < jmin || icoords[1] > jmax)) return NO;
	if (dim == 3 && (icoords[2] < kmin || icoords[2] > kmax)) return NO;
	return YES;
}	/* end InDomainIndex */
	...
	int *n)
{
	...
	for (i = 0; i < M; ++i)
	{
	    if (!cell_error[i].mark_delete)
		continue;
	    for (j = i; j < M-1; ++j)
		cell_error[j] = (CELL_ERROR)cell_error[j+1];
	    i--;
	    M--;
	}
	*n = M;
...
static boolean nextIcomb(int*,int,int);

void C_CARTESIAN::checkVolume()
{
	CELL_PART *cell_part = field->cell_part;
	int i,j,k,l,ic;
	double v0,v;
	...
	FT_VectorMemoryAlloc((POINTER*)&cell_error,size,sizeof(CELL_ERROR));
	n = 0;

	{
	    ...
	    {
		ic = d_index2d(i,j,top_gmax);
		v = cell_part[ic].vol_new[0] + cell_part[ic].vol_new[1];	
		total_vol += v;
		error = (v - v0)/v0;
		total_error += error;
		if (fabs(error) > 0.00001)
		{
		    cell_error[n].error = error;
		    cell_error[n].icoords[0] = i;
		    cell_error[n].icoords[1] = j;
		    cell_error[n].mark_delete = NO;
		    n++;
		}
	    }
	    printf("Step 1: total_error = %20.14f\n",total_error);
	}
	pp_global_sum(&total_error,1);
	printf("global total_error = %20.14f\n",total_error);
	for (i = 2; i < 20; ++i)
	{
	    if (n < i) break;
	    icomb[0] = -1;
	    while (nextIcomb(icomb,i,n))
	    {
		error = 0.0;
		for (j = 0; j < i; ++j)
		{
		    error += cell_error[icomb[j]].error;
		    if (cell_error[icomb[j]].mark_delete)
			break;
		}
		if (j < i) continue;
	    	if (fabs(error) < 0.00001)
		    ...
		    for (j = 0; j < i; ++j)
		    {
			cell_error[icomb[j]].mark_delete = YES;
			printf("%20.14f ",cell_error[icomb[j]].error);
		    }
		    printf("\n");
	    	}
	    }
	    pushBack(cell_error,&n);
	    total_error = 0.0;
	    for (j = 0; j < n; ++j)
		total_error += cell_error[j].error;
	    printf("Step %d: total_error = %20.14f\n",i,total_error);
	}
	pp_global_sum(&total_error,1);
	printf("global total_error = %20.14f\n",total_error);
	if (n != 0)
	...
	for (i = 0; i < n; ++i)
	{
	    printf("icoords = %d %d\n",cell_error[i].icoords[0],
				cell_error[i].icoords[1]);
	}
	return;
}	/* end checkVolume */

static boolean nextIcomb(
	int *icomb,
	int ncomb,
	int N)
{
	...
	for (i = 0; i < ncomb; ++i)
	{
	    if (icomb[ncomb-i-1] < N-i-1)
	    {
		icomb[ncomb-i-1] += 1;
		for (j = ncomb-i; j < ncomb; ++j)
		    icomb[j] = icomb[j-1] + 1;
		return YES;
	    } 
	}
