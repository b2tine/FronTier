        ...
        double              *V)
{
	...
	if (cRparams->add_curvature)
        ...
        dn = 2.0*grid_size_in_direction(nor,h,dim);
        for (i = 0; i < dim; ++i)
            p1[i] = p0[i] + nor[i]*dn;
	...
	CRT_PARAMS *cRparams)
{
	...
	if (CursorAfterStringOpt(infile,"Initial gap:"))
	{
	...
	(void) printf("The default scheme is second order Implicit (IM)\n");
	...
	char *inname)
{
	...
	case 'O':
	    ...
	    case 'g':
	    case 'G':
		sb_params.bdry_type = GROWING_BODY_BOUNDARY;
		break;
	    default:
	...
	if (first)
        {
            ...
            infile = fopen("sample/fuel-1.mesh","r");
	...
	Front *front)
{
	...
	for (i = 0; i < dim; ++i)
	{
	    if (FT_RectBoundaryType(front,i,0) == PERIODIC_BOUNDARY)
	    	continue;
	    if (coords_min[i] - L[i] < h[i] ||
	    	U[i] - coords_max[i] < h[i]) 
	    	bdry_reached = YES;
	}
#if defined (__MPI__)
...
extern int crystal_find_state_at_crossing(
	Front *front,
	int *icoords,
        GRID_DIRECTION dir,
        int comp,
...
}       /* crystal_find_state_at_crossing */
